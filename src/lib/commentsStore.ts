...
export const addCommentToDb = async (commentData: AddCommentData): Promise<Comment> => {
  ...
  console.log(`commentsStore: Adding new comment for article_id ${article_id} by author "${author_name}"`);

  try {
    // Always insert a new comment
    console.log(`commentsStore: Inserting new comment for author "${author_name}" on article ${article_id}.`);
    ...
    console.error('commentsStore: Error adding comment in DB:', error);
    throw error;
  }
};
